...
class Application @Inject()(ws: WSClient) extends Controller {
  ...
  def rootremote = Action.async {
    ...
    val res: Future[Result] = for {
      cc <- carsCounterResponse.map(counterFromJsonResponse)
      mc <- motoCounterResponse.map(counterFromJsonResponse)
    }yield Ok(index(cc+mc))


    val lst: Future[List[WSResponse]] = Future.sequence(List(carsCounterResponse,motoCounterResponse))
    val lsti=lst.map(_.map(counterFromJsonResponse))
    val r: Future[Int]
