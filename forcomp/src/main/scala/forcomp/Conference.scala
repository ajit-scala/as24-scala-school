...
case class Track(talks: Seq[Talk]) {
  ...
  def talksOn(theme: String): Seq[Talk] = talks.filter(x=>x.themes(theme)) // TODO implement using .filter

  /** the talk in this track at the given time */
  def talkAt(time: Time): Option[Talk] = talks.find(x=>x.time==time) // TODO implement using .find

  /** all themes of talks in this track */
  def themes: Set[String] = talks.flatMap(x=>x.themes).to[Set] // TODO implement using .flatMap and .to[Set]
  def themes1: Set[String] = talks.to[Set].flatMap(x=>x.themes) // TODO implement using .flatMap and .to[Set]
...
case class Schedule(tracks: Set[Track]) {
  ...
  // all talks
  def talks: Set[Talk] = tracks.flatMap(_.talks) // TODO implement using .flatMap
  ...
  def themeCount: Map[String, Int] = {
    val allThemes = talks.to[Seq].flatMap(_.themes)
    allThemes.groupBy(identity).map(p=>(p._1,p._2.size))
  }

  def themeCount2: Map[String, Int] = {
    for{
  ...
  // Optional task; write tests along with the implementation.
   def themeRank: List[String] = {
    themeCount2.to[List].sortBy(_._2).reverse.map(_._1)
  }
}
